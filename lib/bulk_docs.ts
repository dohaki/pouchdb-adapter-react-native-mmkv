...
import {
  ...
  forMeta,
  forSequence,
} from "pouchdb-adapter-asyncstorage/src/keys";
...
export default function (db, req, opts, callback) {
  ...
  const mapRequestDoc = (doc) => {
    const parsedDoc = parseDoc(doc, newEdits, db.opts);
  ...
  const getChange = (oldDoc, newDoc) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      processAllAttachments(newDoc.data)
        .then((attachments) => {
          const change: any = oldDoc ? getUpdate() : getInsert();
    ...
    Promise.all(promises)
      .then((changes: any[]) => {
        ...
        changes.forEach((change: any) => {
          ...
          change.attachments &&
            change.attachments.forEach((attachment: any) => {
